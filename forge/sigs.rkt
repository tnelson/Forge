...
; Commands
(provide sig relation fun const pred run #|test|# #|check|# display inst with)
...
(define (get-sigs run-or-state [relation #f])
  ...
  (if relation
      (map (curry get-sig state) (Relation-sigs relation))
...
; send-to-kodkod :: Run-spec -> Stream<model>
