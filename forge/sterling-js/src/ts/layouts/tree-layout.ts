...
export class TreeLayout {
    ...
    constructor (svg, preferences?: TreeLayoutPreferences) {
    ...
    redraw () {
        ...
        if (this._root) this._update(this._root);
    ...
    _exit_nodes (selection, transition, source?) {
        ...
        if (!source) selection.each(d => this._data.delete(unique_id(d)));
...
function apply_attributes (selection, attributes: Object) {
