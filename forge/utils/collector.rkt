...
; Generic recursive-descent collector for Forge AST
; Methods in this module accept a lambda that
;   - accepts context, an AST node, and a list of partial results for the subnodes of that node
;   - produces a list as the partial result for that node
; We don't currently have contracts on the result type; enforcement is up to the caller.
...
;
; a-node: the formula being visited
; fn: the function to call
; run-or-state: run context, if any
; quantvars: quantifier-binding context, if any
(define/contract (collect-for-node a-node fn #:run [run-or-state #f] #:quantvars [quantvars '()])  
  (->* (node? 
         (-> ((or/c Run? State? Run-spec?) node?) 
             (listof any/c)
             list?))
       #:rest
