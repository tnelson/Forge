    ...
    [(node/formula/quantified info quantifier decls form)
     ...
     (define new-quantvars
       (for/fold ([quantvars quantvars])
                 ([decl decls])
         (define decl-v (car decl))
         (define decl-m (car (cdr decl)))
         (define decl-d (car (cdr (cdr decl))))
         (define new-quantvars (cons decl-v quantvars))
         ; Quantified variables of arity >1 are always "set" quantified. 
         (print-cmd-cont (format "[~a : ~a " (v (get-sym decl-v) #;(get-var-idx (car decl) new-quantvars)) 
                                             (if (or (@> (node/expr-arity decl-v) 1)
                                                     (equal? "set" decl-m)) 
                                                 "set" "one")))
         (interpret-expr run-or-state decl-d relations atom-names new-quantvars)
         (print-cmd-cont "] ")
         new-quantvars))
     (print-cmd-cont ") ")
