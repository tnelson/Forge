...
(test oneno #:preds [(iff oneVarno oneVarnoDisj)] #:expect checked)
...
(pred twoVarnoDisj
      (no #:disj ([n1 Node] [n2 Node])
        (in n1 (join n2 edges))))
(test twono #:preds [(iff twoVarno twoVarnoDisj)] #:expect checked)
...
(pred manyVarnoDisj 
    (no #:disj ([n0 Node] [n1 Node] [n2 Node] [n3 Node])
        (&& (in (-> n1 n2) edges)
            (in (-> n3 n0) edges))))
(test manyno #:preds [(iff manyVarno manyVarnoDisj)] #:expect checked)
